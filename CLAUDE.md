...
## 18. KOTSA 실패의 두 가지 원인 — 구별해서 대응할 것

`vehicle=failure` 는 원인이 둘이고 **대응이 정반대**다. 반드시 실제 키로 호출해 구별한다.

| 증상(로컬에서 실제 키로 호출) | 원인 | 대응 |
|---|---|---|
| `HTTP 504` + `SERVICETIMEOUT_ERROR`(code 05) | **공급자 백엔드 장애** | 재실행 무의미. 회복 대기 (§15 수동 입력) |
| `HTTP 200` + `NORMAL_CODE` (정상 응답) | **러너→apis.data.go.kr 경로 차단** | 재실행하면 될 수도. 안 되면 **로컬 수집 후 커밋** |

2026-08·09 실패는 **후자**였다. CI 로그는 세 번 모두 `ConnectTimeoutError`(응답 자체를 못 받음)인데,
같은 시각 로컬에서는 17개 시도 전부 **0.46초 평균**으로 정상 응답했다. 공급자는 멀쩡한데 러너에서만
막힌 것이다. `SERVICETIMEOUT_ERROR`(공급자가 응답은 함)와 `ConnectTimeoutError`(응답 자체 없음)를
혼동하지 말 것.

### 적용한 개선
- `fetch_count`: 타임아웃 30→60초, 재시도 3→5회, 고정 10초 → **지수 백오프(5/10/20/40)**
- `preflight()`: 85회 본수집 전 단건으로 연결 확인 → 실패 시 즉시 중단하고 원인 안내
  (기존엔 첫 시도부터 재시도를 반복해 111초 쓰고 전량 폐기)
- 워크플로 vehicle 스텝: **180초 간격 2회 시도** (간헐적 차단이라 시간차 재시도가 유효)
- **stdout UTF-8 강제**: Windows cp949 에서 `—`/`→` 가 `UnicodeEncodeError` 를 내
  **멱등 재실행 경로가 죽던 버그** 수정. §9 의 "재실행하면 안전" 전제가 로컬에선 깨져 있었다.

### 그래도 막히면 (최후 수단)
로컬에서 `EV_REG_API_KEY` 지정 후 `TARGET_YYYYMM=YYYYMM python scripts/fetch_ev_registration.py`
→ 생성된 `data/ev_registration/{ym}.json` + `cumulative.json` 커밋 → `build_dashboard_data.py`.
멱등성 가드가 있어 같은 월 재실행은 안전하다(중복합산 없음 — 실제로 검증함).

### 미해결 취약점
85회를 한 트랜잭션으로 처리하고 1건만 실패해도 전량 폐기하는 구조는 그대로다(체크포인트 없음).
preflight 로 "아예 못 나가는" 경우는 걸러지지만, 중간 실패는 여전히 전체를 날린다.
